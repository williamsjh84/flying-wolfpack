...
export const metadata: Metadata = {
  title: "Travel Hacks",
  description: "Hard-won lessons from 200+ travel days as a family.",
};
    ...
    <PageWrapper>
      ...
      <section className="bg-warm-white py-24">
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          <SectionHeader eyebrow="Tips & Tactics" headline="All Travel Hacks" className="mb-12" />
          <div className="mb-10 flex flex-wrap gap-2">
            {CATEGORIES.map((cat, i) => (
              <button key={cat} className={`border px-4 py-2 font-sans text-sm tracking-wide transition-colors ${i === 0 ? "border-ink bg-ink text-warm-white" : "border-mist text-ink/60 hover:border-earth hover:text-ink"}`}>{cat}</button>
            ))}
          </div>
